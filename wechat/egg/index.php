...
<div class="grid">
    ...
    <div class="block">
        ...
        <?php foreach ( $wxchdata as $v ) { ?>
            <p><?php echo $v['level'] ?>---<?php echo $v['prize_name'] ?> <?php if ( $v['state'] == 'yes' ) {
...
<script>
    $(function () {
        ...
        function wxch_result() {
            ...
            $.getJSON("/wechat/prize_data.php?pid=<?php echo $pid.'&pzfun='.$prize['pzfun'].'&wxid='.$wxid; ?>", function (data) {
                $("#mask").show();
                if (data.msg === 1) {
                    $("#content").html(data.prize);
                    $.post("/wechat/api.php?act=egg", {
                        pid: data.pid,
