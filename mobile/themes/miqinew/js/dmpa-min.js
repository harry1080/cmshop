...
var DMPA = DMPA || (function () {
        ...
        function sha1(str) {
            var rotate_left = function (n, s) {
                    ...
                    return str
                }, blockstart, i, j, W = [], H0 = 1732584193, H1 = 4023233417, H2 = 2562383102, H3 = 271733878,
                H4 = 3285377520, A, B, C, D, E, temp, str_len, word_array = [];
            str = utf8_encode(str);
            str_len = str.length;
        ...
        function Tracker(siteId) {
            var registeredHooks = {}, fields = {}, mapFields = {
                ...
                customVariables = false, customVariablesPage = {}, customVariableMaximumLength = 200,
                browserFeatures = {}, linkTrackingInstalled = false, activityTrackingInstalled = false,
                lastActivityTime, lastButton, lastTarget, hash = sha1, domainHash, visitorUUID, visitorUUIDEncrypt,
                pageURLBeforeRegenerate, configHaIgnoreClass = "ha_ignore", configHaClickClass = "hwa-click",
                perf_event_q = [], perf_send_status = "sleeping", perf_ajaxpage_in = 0, perf_ajaxpage_out = 0,
                perf_ajaxpage_sended = false, perf_ajaxpage_min_time = 0, perf_ajaxpage_max_time = 0,
                perf_ajaxpage_firstloadtime = 0, ajaxthreshold_time = 0,
            ...
            function getRequest(request, customData, pluginMethod, currentEcommerceOrderTs) {
                var i, now = new Date(), nowTs = Math.round(now.getTime() / 1000), newVisitor, uuid, visitCount,
                    createTs, currentVisitTs, lastVisitTs, lastEcommerceOrderTs, referralTs, referralUrl,
                    referralUrlMaxLength = 1024, currentReferrerHostName, originalReferrerHostName,
                    customVariablesCopy = customVariables, idname = getCookieName("id"), sesname = getCookieName("ses"),
                    refname = getCookieName("ref"), cvarname = getCookieName("cvar"), datasend = null,
                    id = loadVisitorIdCookie(), ses = getCookie(sesname),
                    attributionCookie = loadReferrerAttributionCookie(),
                    currentUrl = configCustomUrl || locationHrefAlias, campaignNameDetected, campaignKeywordDetected;
                if (configCookiesDisabled) {
                    ...
                    try {
                        ...
                        hwa_meld.after(Jalor.Page, ["forward", "_forward", "forwardMethod", "forwardIntervalMethod"], function (result) {
                            try {
                                ...
                                var sesname = getCookieName("ses"), _id = loadVisitorIdCookie(),
                                    _ses = getCookie(sesname), _uuid = _id[1];
            ...
            return {
                hook: registeredHooks, getHook: function (hookName) {
                    ...
                    var sesname = getCookieName("ses"), id = loadVisitorIdCookie(), ses = getCookie(sesname),
                        uuid = id[1];
