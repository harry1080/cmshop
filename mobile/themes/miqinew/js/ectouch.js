...
//删除购物栏中的某个商品
var delCart = function (res) {
    //询问框
    layer.open({
        content: res.lang
        ,btn: ['确定', '取消']
        ,yes: function(index){
            location.href='flow.php?step=drop_goods&id='+res.rel;
            //location.reload();
            layer.close(index);
        }
    });
...
//删除联系人收货地址
var delConsignee = function (res) {
    //询问框
    layer.open({
        content: res.lang
        ,btn: ['确定', '取消']
        ,yes: function(index){
            location.href='flow.php?step=drop_consignee&id='+res.value;
            //location.reload();
            layer.close(index);
        }
    });
