<?php
     ...
     * 云动商圈购物流程
     * ============================================================================
     * 版权所有 2005-2010 中移铁通智能产品分公司，并保留所有权利。
    ...
    if ( $_REQUEST['step'] == 'add_to_cart' ) {
        ...
        $quick = $_POST['quick']; //是否立即购买

        $spec = $_POST['spec'];//商品规格
        $parent = $_POST['parent'];//商品规格

        $number = $_POST['number']; //购买的数量
       ...
       if($number > 0){
            // 更新：添加到购物车
            if ( addto_cart( $goods_id , $number , $spec , $parent ) ) {
     ...
     */
         ...
         */
            ...
            if ( $_SESSION['user_id'] > 0 ) {
                ...
                //by tongwenping@163.com 有几个联系人显示几个
        ...
        if ( drop_consignee( $consignee_id ) ) {
            ...
            //数据库里没有收货人信息了但是还存在缓存，清除缓存
            if(isset( $_SESSION['flow_consignee'])){
                $_SESSION['flow_consignee'] =null;
                ecs_header( "Location: flow.php?step=consignee\n" );
            }else{
     ...
     */
        ...
        if ( isset( $payment_list ) ) {
            foreach ( $payment_list as $key => $payment ) {
                if ( $payment['is_cod'] == '1' ) {
                    $payment_list[$key]['format_pay_fee'] = '<span id="cod_fee">' . $payment['format_pay_fee'] . '</span>';
                }
