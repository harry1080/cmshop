...
if (empty ( $sign ) || empty ( $sign_type ) || empty ( $biz_content ) || empty ( $service ) || empty ( $charset )) {
	writeLog ( "sign: " . var_export ( $sign, true ) );
	writeLog ( "sign_type: " . var_export ( $sign_type, true ) );
	writeLog ( "biz_content: " . var_export ( $biz_content, true ) );
	writeLog ( "service: " . var_export ( $service, true ) );
	writeLog ( "charset: " . var_export ( $charset, true ) );
...
class Gateway {
	public function verifygw($is_sign_success) {
		...
		if ($EventType == "verifygw") {
			...
			// print_r ( $config );
			if ($is_sign_success) {
				$response_xml = "<biz_content>" . $as->getPublicKeyStr ( $config ['merchant_public_key_file'] ) . "</biz_content><success>true</success>";
