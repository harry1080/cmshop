<?php
    ...
    class Common_util_pub
    {
        ...
        public function ToUrlParams( $paraMap )
        {
            $buff = "";
            foreach ( $paraMap as $k => $v ) {
                if ( $k != "sign" && $v != "" ) {//过滤掉自定义的code参数（异步通知重要）
                    $buff .= $k . "=" . $v . "&";
                }
            }
...
//		$String = $this->formatBizQueryParaMap($Parameters, false);
            ...
            $this->log( ROOT_PATH . '/data/wx_new_log.txt' , 'string:' . $String );//错误
        ...
        {
            $fp = fopen( $file , 'ab+' );
            fwrite( $fp , '-----------' . local_date( 'Y-m-d H:i:s' ) . '--mobile---------------' );
            fwrite( $fp , $txt );
            fwrite( $fp , "\r\n\r\n\r\n" );
            fclose( $fp );
        }
        ...
        {
            $this->log( ROOT_PATH . '/data/wx_new_log.txt' , 'xmlData:' . $xml );//正确
            //将XML转为array
    ...
    {
        public $data = array();//接收到的数据，类型为关联数组
        ...
        function checkSign()
        {
            ...
            $sign = $this->getSign( $this->data );//本地签名
            $sign_xml = $this->arrayToXml( $sign );//本地签名

            $this->log( ROOT_PATH . '/data/wx_new_log.txt' , 'xmlData:' . $sign_xml );//正确
            $this->log( ROOT_PATH . '/data/wx_new_log.txt' , 'mobile:getSign:' . $sign );//错误
            $this->log( ROOT_PATH . '/data/wx_new_log.txt' , 'mobile:sign:' . $this->data['sign'] );//正确

            if ( $this->data['sign'] == $sign ) {
                $this->log( ROOT_PATH . '/data/wx_new_log.txt' , '验证签名成功SUCCESS' );//正确

                return true;
            }
            $this->log( ROOT_PATH . '/data/wx_new_log.txt' , '验证签名失败 Failed' );//正确
    ...
    class JsApi_pub extends Common_util_pub
    {
        ...
        public function mygetSign()
        {
            $xml = '<xml><appid><![CDATA[wxfb2d35da823285a0]]></appid>
<attach><![CDATA[118]]></attach>
<bank_type><![CDATA[CFT]]></bank_type>
<cash_fee><![CDATA[2]]></cash_fee>
<fee_type><![CDATA[CNY]]></fee_type>
<is_subscribe><![CDATA[Y]]></is_subscribe>
<mch_id><![CDATA[1342269701]]></mch_id>
<nonce_str><![CDATA[16dr1x6bamsn35zguahv9gttsud6xvc0]]></nonce_str>
<openid><![CDATA[o_fypwHt4F0uJu-i8lheviEJ0AzE]]></openid>
<out_trade_no><![CDATA[2017010913210]]></out_trade_no>
<result_code><![CDATA[SUCCESS]]></result_code>
<return_code><![CDATA[SUCCESS]]></return_code>
<sign><![CDATA[9A0F29F2BEAE2270D8637EF9FD8F60D5]]></sign>
<time_end><![CDATA[20170110001052]]></time_end>
<total_fee>2</total_fee>
<trade_type><![CDATA[JSAPI]]></trade_type>
<transaction_id><![CDATA[4009752001201701095786449276]]></transaction_id>
</xml>';
