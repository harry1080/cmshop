<?php
    ...
    function pack_list()
    {
        $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table( 'pack' ) . ' WHERE hide = 0 ';
        $res = $GLOBALS['db']->query( $sql );
        ...
        while ( $row = $GLOBALS['db']->fetchRow( $res ) ) {
            ...
            //是否显示
            $row['format_hide'] = price_format( $row['hide'] , false );
            $list[] = $row;
        }
    ...
    function card_list()
    {
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table( 'card' ) . ' WHERE hide = 0 ';
        $res = $GLOBALS['db']->query( $sql );
