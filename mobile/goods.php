<?php
    ...
    if ( !empty( $_REQUEST['act'] ) && $_REQUEST['act'] == 'price' ) {
        ...
        if ( $goods_id == 0 ) {
            ...
            $results = $shop_price * $number;
            $res['result'] = "&yen;" . $results;
        }
    ...
    //获得商品的详情 by tongwenping
    if ( !empty( $_REQUEST['act'] ) && $_REQUEST['act'] == 'getGoodsDetail' ) {
        include( 'include/cls_json.php' );
        $goods_id_1 = $_REQUEST['goods_id'];
        $json = new JSON;
        $res = array( 'success' => 'true' );
        $goods = get_goods_info( $goods_id_1 );
...
//            var_dump($linked_goods);
            ...
            $smarty->assign( 'pictures_num' , count( $pictures ) );
...
//-- PRIVATE FUNCTION
    ...
    function get_linked_goods( $goods_id )
    {
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img,g.original_img, g.shop_price AS org_price, ' .
            ...
            'g.market_price, g.promote_price, g.promote_start_date, g.promote_end_date,g.promote_word,g.promLink4Wap ' .
...
// 统计商品的评论数
    ...
    function get_comment_res( $goods_id )
    {
        ...
        $res['goodCount'] = 3713;
        $res['goodRate'] = 1.0;
        $res['hasImageCount'] = 43;


        $comments['commentId'] = '24402045365834';
        $comments['commentLevel'] = "好评";
        $comments['content'] = "物流也很快，手机包装不错！";
        $comments['creationTime'] = "2017-12-08 10:20:45+0800";
        $comments['gradeCode'] = 3;
        $comments['headImage'] = "https://res.vmallres.com/20170623/images/echannel/misc/defaultface_user_small.png";
        ...
        $comments['isTop'] = 1;
        $comments['likes'] = 93;
        $comments['productId'] = '570379791';
        $comments['replies'] = array();
        $comments['score'] = 5;
        $comments['skuCode'] = "2601010014006";
        $comments['tags'] = array();
        $comments['userName'] = "w***i";
