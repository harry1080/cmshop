<?php
    ...
    */
     ...
     * 功能：license 登录
     *
     * @param string $certi_append 配置信息补充数组 array_key 登录信息的key；array_key => array_value；
     *
     * @return  array     $return_array['flag'] = login_succ、login_fail、login_ping_fail、login_param_fail；
     *                    $return_array['request']；
     */
    function license_login( $certi_append )
    {
        // 登录信息配置
        ...
        if ( is_array( $certi_append ) ) {
            foreach ( $certi_append as $key => $value ) {
                $certi[$key] = $value;
            }
        }
        ...
        if ( !empty( $license['certificate_id'] ) && !empty( $license['token'] ) && !empty( $license['certi'] ) ) {
            ...
            $certi['certi_ac'] = make_shopex_ac( $certi , $license['token'] ); // 网店验证签名
            $certi['certi_app'] = 'certi.login'; // 证书方法

            $request_arr = exchange_shop_license( $certi , $license );
            if ( is_array( $request_arr ) && $request_arr['res'] == 'succ' ) {
                $return_array['flag'] = 'login_succ';
                $return_array['request'] = $request_arr;
            } elseif ( is_array( $request_arr ) && $request_arr['res'] == 'fail' ) {
     ...
     *               $return_array['request']；
     */
    function license_reg( $certi_append )
    {
        // 登录信息配置
        ...
        // 注册
        $certi['certi_ac'] = make_shopex_ac( $certi , $certi_append['token'] ); // 网店验证字符串
        $request_arr = exchange_shop_license( $certi , $license );
        if ( is_array( $request_arr ) && $request_arr['res'] == 'succ' ) {
            // 注册信息入库
            ...
            $GLOBALS['db']->query( $sql );
            $sql = "UPDATE " . $GLOBALS['ecs']->table( 'shop_config' ) . "
                SET value = '" . $request_arr['info']['certi'] . "' WHERE code = 'certi'";
            $GLOBALS['db']->query( $sql );
    ...
    function license_check( $append = '' )
    {
        ...
        if ( !empty( $license['certificate_id'] ) && !empty( $license['token'] ) && !empty( $license['certi'] ) ) {
            // license（登录）
            $return_array = license_login( $append );
        } else {
            // license（注册）
            $return_array = license_reg( $append );
        }
