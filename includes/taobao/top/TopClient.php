<?php
    ...
    class TopClient
    {
        ...
        public function execute( $request , $session = null , $bestUrl = null )
        {
            ...
            $mergeParams = array_merge( $apiParams , $sysParams );
            //签名
            $sysParams["sign"] = $this->generateSign( $mergeParams );
            ...
            if ( isset( $respObject->code ) ) {
                ...
                $logger->conf["log_file"] = rtrim( TOP_SDK_WORK_DIR , '\\/' ) . '/' . "taobao_" . $this->appkey . "_" . date( "Y-m-d" ) . ".err.log";
                $logData = array(
                    date( "Y-m-d H:i:s" ) ,
                    $resp,
                );
                $ret = $logger->log( $logData );

                if ( !$ret ) {//失败返回false，成功返回数字
        ...
        public function curl( $url , $postFields = null )
        {
            ...
            if ( is_array( $postFields ) && 0 < count( $postFields ) ) {
                ...
                foreach ( $postFields as $k => $v ) {
                    ...
                    if ( "@" != substr( $v , 0 , 1 ) )
                    {//不是文件上传x-www-form-urlencoded
                        $postBodyString .= "$k=" . urlencode( $v ) . "&";//不是
