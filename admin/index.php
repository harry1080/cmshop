...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
        ...
        $diff = date( 'Y-m-d H:i:s' , time() - 43200 );
        $timediff = API_TIME - $diff;
        if ( $data === false || $timediff < 0 ) {
            ...
            $api_comment = $transport->request( $_CFG['main_api'], $params );
            $data = json_decode($api_comment["body"],true) ;
            $api_str = $data['msg'];
            echo $api_str;
...
//-- Totolist操作
    ...
    elseif ( $_REQUEST['act'] == 'license' ) {
        ...
        if ( isset( $is_ajax ) && $is_ajax ) {
            ...
            switch ( $license['flag'] ) {
                case 'login_succ':
                    if ( isset( $license['request']['info'] ) ) {
                        make_json_result( process_login_license( $license['request']['info'] ) );
                    } else {
                        make_json_error('flag:login_succ info is not set' );
                    }
                    break;

                case 'login_fail':
                case 'login_ping_fail':
                    make_json_error('flag:'.$license['flag']);
                    break;
                ...
                case 'reg_succ':
                    ...
                    switch ( $_license['flag'] ) {
                        case 'login_succ':
                            if ( isset( $license['request']['info'] ) && $license['request']['info']['auth_str'] != '' ) {
                                make_json_result( process_login_license( $license['request']['info'] ) );
                            } else {
                                make_json_error('auth_str is empty or is not set');
                            }
                            break;

                        case 'login_fail':
                        case 'login_ping_fail':
                            make_json_error('flag:reg_succ '.$license['flag']);
                            break;
                    }
                    break;
            ...
            make_json_error( 'is_ajax is empty or is not set');
        }
    }
