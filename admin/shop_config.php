...
//-- 提交   ?act=post
    ...
    elseif ( $_REQUEST['act'] == 'post' ) {
        ...
        $params = '';
        $params .= "url=" . urlencode( $ecs->url() );
        $params .= "&shop_name=" . urlencode( $_CFG['shop_name'] );
        $params .= "&shop_title=" . urlencode( $_CFG['shop_title'] );
        $params .= "&shop_desc=" . urlencode( $_CFG['shop_desc'] );
        $params .= "&shop_keywords=" . urlencode( $_CFG['shop_keywords'] );
        $params .= "&country=" . urlencode( $shop_country ) . "&province=" . urlencode( $shop_province ) . "&city=" . urlencode( $shop_city );
        $params .= "&address=" . urlencode( $_CFG['shop_address'] );
        $params .= "&qq=$_CFG[qq]&ww=$_CFG[ww]&ym=$_CFG[ym]&msn=$_CFG[msn]";
        $params .= "&email=$_CFG[service_email]&phone=$_CFG[service_phone]&icp=" . urlencode( $_CFG['icp_number'] );
        $params .= "&version=" . VERSION . "&language=$_CFG[lang]&php_ver=" . PHP_VERSION . "&mysql_ver=" . $db->version();
        $params .= "&charset=" . EC_CHARSET;

        $transport = new transport;
        $api_comment = $transport->request( $_CFG['record'] , $params );
