...
if ($_REQUEST['act'] == 'list')
{
    ...
    //打开 $lang_path==zh_cn/en_us/zh_fw 目录
    ...
    while ($file = @readdir($lang_dir))
    {
        if (substr($file, -3) == "php")
        {
            ...
            $lang_paths = $lang_path."/".$file;
            $lang_dirs    = @opendir($lang_paths);
            while ($files = @readdir($lang_dirs)){
                if (substr($files, -3) == "php")
                {
                    $filenames = substr($files, 0, -4);
                    $lang_arr[$filenames] = $files. ' - ' .@$_LANG['language_files'][$filenames];
                }
            }
        }
