...
//-- 添加新包装
    ...
    elseif ( $_REQUEST['act'] == 'add' ) {
        ...
        /*插入数据*/
        $sql = "INSERT INTO " . $ecs->table( 'card' ) . "(card_name, card_fee, free_money, card_desc, card_img,hide)
            VALUES ('$_POST[card_name]', '$_POST[card_fee]', '$_POST[free_money]', '$_POST[card_desc]', '$img_name','$_POST[hide]')";
        $db->query( $sql );
...
//-- 编辑包装
    ...
    elseif ( $_REQUEST['act'] == 'edit' ) {
        ...
        $sql = "SELECT card_id, card_name, card_fee, free_money, card_desc, card_img ,hide FROM " . $ecs->table( 'card' ) . " WHERE card_id='$_REQUEST[id]'";
        $card = $db->GetRow( $sql );
        ...
        $param = "card_name = '$_POST[card_name]', card_fee = '$_POST[card_fee]', free_money= $_POST[free_money], card_desc = '$_POST[card_desc]', hide = '$_POST[hide]'";
        /* 处理图片 */
...
//-- ajax编辑免费额度
    ...
    function cards_list()
    {
        ...
        if ( $result === false ) {
            ...
            /* 查询 */
            $sql = "SELECT card_id, card_name, card_img, card_fee, free_money, card_desc,hide" .
